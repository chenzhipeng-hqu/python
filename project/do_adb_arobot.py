...
# except:
#     logger.error(('Following module is needed:')
#     logger.error(('- Tkinter: sudo apt-get install python-tk')
#     logger.error(('- PIL: sudo apt-get install python-imaging-tk')
    # sys.exit()
...
def readAdbResponse(s):
    ...
    if len(resp) != 4:
        logger.error('protocol fault (no status)')
        return False
    ...
    if resp == b'OKAY':
        ...
        resp = s.recv(4)
        if len(resp) < 4:
            logger.error('protocol fault (status len)')
            return False
        else:
            length = int(resp, 16)
            resp = s.recv(length)
            if len(resp) != length:
                logger.error('protocol fault (status read)')
                return False
            else:
                logger.error(resp)
                return False
    else:
        logger.error("protocol fault (status %02x %02x %02x %02x?!)" % (resp[0], resp[1], resp[2], resp[3]))
        return False
    return False
...
def getInt(tbuf=None):
    if (tbuf != None):
        ...
        if len(tbuf) < 4:
            logger.error('buff len < 4')
            return 0
        else:
            if DEBUG:
                logger.debug('parse: %02x %02x %02x %02x' % (tbuf[0], tbuf[1], tbuf[2], tbuf[3]))
            intnum = tbuf[0]
            intnum = intnum + tbuf[1] * 0x100
            intnum = intnum + tbuf[2] * 0x10000
            intnum = intnum + tbuf[3] * 0x1000000
            if DEBUG:
                logger.debug('INT: %08x' % intnum)
            return intnum
    else:
        return 0
...
def readHeader(tfb, ver, buf):
    if DEBUG:
        logger.debug('readHeader: ver = %d' % ver)
    if ver == 16:
        tfb.fb_bpp = 16
...
def send_touch_event(action, x0, y0, x1=None, y1=None):
    ...
    if action == 'tap':
        ...
        if 'Error' in resp:
            logger.error('Not support tap command')
            # get tp device
            tp = get_touch_event()
            if tp == None:
                return
...
class send_key_thread(threading.Thread):
    ...
    def __init__(self, key):
        if DEBUG:
            logger.debug('send_key_thread init')
    ...
    def sendKey(self):
        if DEBUG:
            logger.debug('send_key: %s' % self.__key)
    ...
    def run(self):
        if DEBUG:
            logger.debug('send_key_thread run')
        self.sendKey()
    def stop(self):
        if DEBUG:
            logger.debug('stop send_key_thread')
...
class KeypadApplication(tk.Frame):
    ...
    def sendKey(self, event=None):
        if DEBUG:
            logger.debug(event.widget.winfo_name())
...
class LcdApplication(tk.Frame):
    ...
    def __init__(self, master=None):
        if DEBUG:
            logger.debug('LcdApplication: __init__')
        if master == None:
            master = tk.Tk()
    ...
    def createlcd(self):
        # creat label as lcd
        if DEBUG:
            logger.debug('LcdApplication: createlcd')
    ...
    def rightclick_label(self, event=None):
        ...
        logger.debug("rotate: %d" % self.__rotate)
        ...
        while self.__keepupdate:
            ...
            if readAdbResponse(s):
                ...
                if '\t' in dev_info:
                    ...
                    dev_sn = ''
                if DEBUG:
                    logger.debug('dev serial: %s' % dev_sn)
            ...
            if not readAdbResponse(s):
                ...
                if DEBUG:
                    logger.debug('ready to transport')
                req_msg = 'framebuffer:'
                s.sendall('%04x' % len(req_msg))
                s.sendall(req_msg)
                if not readAdbResponse(s):
                    ...
                    if fbver == 16:
                        ...
                        hdrsize = 0;
                    if DEBUG:
                        logger.debug('fb header size: %d' % hdrsize)
                    # read the header
                    ...
                    if DEBUG:
                        logger.debug('bpp: %d' % myfb.fb_bpp)
                        logger.debug('size: %d' % myfb.fb_size)
                        logger.debug('width: %d' % myfb.fb_width)
                        logger.debug('height: %d' % myfb.fb_height)
                        logger.debug('red_offset: %d' % myfb.red_offset)
                        logger.debug('red_length: %d' % myfb.red_length)
                        logger.debug('blue_offset: %d' % myfb.blue_offset)
                        logger.debug('blue_length: %d' % myfb.blue_length)
                        logger.debug('green_offset: %d' % myfb.green_offset)
                        logger.debug('green_length: %d' % myfb.green_length)
                        logger.debug('alpha_offset: %d' % myfb.alpha_offset)
                        logger.debug('alpha_length: %d' % myfb.alpha_length)
                    # read fb buffer
                    rcvcnt = 0
                    readbyte = 0
                    imagebuff = []
                    while True:
                        ...
                        if DEBUG:
                            logger.debug('read byte: %d' % len(resp))
                        rcvcnt = rcvcnt + len(resp);
                        imagebuff.extend(resp)
                        if len(resp) == 0:
                            break
                    if DEBUG:
                        logger.debug('total rcv byte: %d' % rcvcnt)
                    reply = s.recv(10)
                    s.close()
                    ...
                    lcd_h = image.size[1]
                    if DEBUG:
                        logger.debug('LCD size: %d x %d' % (lcd_w, lcd_h))
                    factor_w = 1.00
                    factor_h = 1.00
                    if lcd_w > max_lcd_w:
                        img_w = max_lcd_w
                    ...
                    if DEBUG:
                        logger.debug('Image size: %d x %d' % (img_w, img_h))
...
class arobot_keys(threading.Thread):
    ...
    def run(self):
        ...
        if DEBUG:
            logger.debug('exit arobot_keys mainloop')
    def stop(self):
        if DEBUG:
            logger.debug('stop arobot_keys')
...
class arobot_lcd(threading.Thread):
    ...
    def run(self):
        ...
        self.__tkapp.mainloop()
        if DEBUG:
            logger.debug('exit arobot_lcd mainloop')
        self.__tkapp.stop()
    def stop(self):
        if DEBUG:
            logger.debug('stop arobot_lcd')
        self.thread_stop = True
        if self.__tkapp != None:
            self.__tkapp.stop()
            self.__tkapp.quit()
...
def usage():
    logger.debug( '--------------------------------------------')
    logger.debug( 'Arobot 3.1')
    logger.debug( 'This is a tool to control Android device via ADB')
    logger.debug( 'usage: python %s [option]' % sys.argv[0])
    logger.debug( 'option:')
    logger.debug( '  -keypad     run keypad')
    logger.debug( '  -lcd        run lcd')
    logger.debug( '--------------------------------------------')
