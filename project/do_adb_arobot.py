...
def readAdbResponse(s):
    if s != None:
        ...
        if DEBUG:
            print('resp[%d]: %s' % (len(resp), repr(resp)))
    ...
    if resp == b'OKAY':
        ...
        print(
        "protocol fault (status %02x %02x %02x %02x?!)", (resp[0], resp[1], resp[2], resp[3]))
...
def getInt(tbuf=None):
    if (tbuf != None):
        ...
        if len(tbuf) < 4:
            ...
            if DEBUG:
                print(
                'parse: %02x %02x %02x %02x' % (tbuf[0], tbuf[1], tbuf[2], tbuf[3]))
...
class LcdApplication(tk.Frame):
    ...
    def click_label(self, event=None):
        ...
        if event.type == '4':
            ...
            if DEBUG:
                print(
                'Release at: (%d, %d)' % (event.x, event.y))
        ...
        if abs(self.__start[0] - self.__end[0]) < 2 and abs(self.__start[1] - self.__end[1]) < 2:
...
def updatelcd_sock(self):
 ...
 \
            ...
            if DEBUG:
                ...
                print(
                'width: %d' % myfb.fb_width)

                print(
                'height: %d' % myfb.fb_height)

                print(
                'red_offset: %d' % myfb.red_offset)

                print(
                'red_length: %d' % myfb.red_length)

                print(
                'blue_offset: %d' % myfb.blue_offset)

                print(
                'blue_length: %d' % myfb.blue_length)

                print(
                'green_offset: %d' % myfb.green_offset)

                print(
                'green_length: %d' % myfb.green_length)

                print(
                'alpha_offset: %d' % myfb.alpha_offset)

                print(
                'alpha_length: %d' % myfb.alpha_length)
...
def usage():
    ...
    print(
    'This is a tool to control Android device via ADB')

    print(
    'usage: python %s [option]' % sys.argv[0])
