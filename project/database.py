    ...
    def valid_stock(self, dates):
        ...
        for i in range(0,len(value_code)):
            ...
            try:
                ...
                # if opens2 < close1 and close2 > opens1 and volume1 > (volume2)\
                if opens2 < close1 and close2 > opens1 \
                        and p_change2 < -2.5 and p_change1 > 9.8:
                # if p_change2 < -5.5 and p_change1 > 9.8:
    ...
    def find_gap(self, dates):
        ...
        for i in range(0,len(value_code)):
            ...
            try:
                ...
                #加入这两天的数据满足昨天下跌超过2%，而且今天的开盘价低于昨天的收盘价，且今天的收盘价高于昨天的收盘价，就满足阳包阴的条件
                if low1 > high2 and p_change2 < 0 and p_change1 < 9.8:
                # if p_change2 < -5.5 and p_change1 > 9.8:
                    # logger.info('%s票%s的开盘价是%s'%(value_code[i][0],today,opens1))
                    # logger.info('%s票%s的收盘价是%s'%(value_code[i][0],today,close1))
                    # logger.info('%s票%s的涨幅是%s'%(value_code[i][0],today,p_change1))
        ...
        with open('./datas/report_gap/%s.txt' % (today), 'w') as fp:
            fp.write('总共找到%d支满足条件的股票:%s' % (a, count))
...
if __name__ == '__main__':
    ...
    logger.info(today)
    # db.valid_stock('2020-06-19')
    db.valid_stock(today)
    # db.find_gap('2020-06-19')
