...
class SerialBus(BusABC):
    ...
    def send(self, msg, timeout=None):
        ...
        byte_msg.append(0x66)
        byte_msg.append(0xCC)
		#print(msg.dlc)
        length = 2 + 4 + 1 + msg.dlc + 1
        byte_msg.append((length>>8) & 0xff)
        byte_msg.append(length & 0xff)
        byte_msg.append(0x30)
        byte_msg.append(0x03)
        for i in range(0, 4):
            byte_msg.append(a_id[3-i])
        byte_msg.append(msg.dlc)
        for i in range(0, msg.dlc):
            byte_msg.append(msg.data[i])
        byte_msg.append((sum(byte_msg[2:])&0xff))
